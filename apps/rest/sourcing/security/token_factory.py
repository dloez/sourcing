...
def create_token(data: dict) -> Token:
    access_token = _create_token(
        data, ACCESS_SECRET_KEY, timedelta(minutes=ACCESS_TOKEN_EXPIRE_MINUTES)
    )
    refres_token = _create_token(
        data, REFRESH_SECRET_KEY, timedelta(minutes=REFRESH_TOKEN_EXPIRE_MINUTES)
    )
    return Token(
        access_token=access_token, token_type="bearer", refresh_token=refres_token
    )
