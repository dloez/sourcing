...
async def register(user: RegisterUser) -> ResponseUser:
    if await User.find_by_email(user.email):
        raise HTTPException(
            status_code=409, detail=f"User '{user.email}' already exists"
        )
    user.password = hash_password(user.password)

    new_user = await users_collection.insert_one(user.model_dump(exclude=["id"]))
    if not new_user:
...
@router.get("/me", response_model=ResponseUser, response_model_by_alias=False)
async def me(
